def calibracao_soma(linhas):
    ...
    for linha in linhas:
        ...
        for numero in linha:
            if numero.isdigit():
                primeiro_numero = int(numero)
                break

        for numero in reversed(linha):
            if numero.isdigit():
                ultimo_numero = int(numero)
                break

        if primeiro_numero != -1 and ultimo_numero != -1:
            valor = primeiro_numero * 10 + ultimo_numero
            soma += valor
...
def main():
    ...
    calibracao_soma_valor = calibracao_soma(linhas)
    print("Soma do primeiro e ultimo número:", calibracao_soma_valor)
